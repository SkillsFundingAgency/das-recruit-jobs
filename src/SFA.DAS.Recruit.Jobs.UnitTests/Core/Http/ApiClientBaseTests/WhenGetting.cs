...
public class WhenGetting
{
    ...
    public async Task Then_The_Request_Is_Made_Correctly()
    {
        ...
        // act
        await sut.GetAsync<NoResponse>(new GenericGetRequest("/api/foo"));
    ...
    public async Task Then_The_Response_Is_Processed_Correctly(NotificationEmail email)
    {
        ...
        // act
        var result = await sut.GetAsync<NotificationEmail>(new GenericGetRequest("http://localhost:8080/api/foo"));
    ...
    public async Task Then_The_Error_Content_Is_Returned_Correctly(NotificationEmail email)
    {
        ...
        // act
        var result = await sut.GetAsync<NotificationEmail>(new GenericGetRequest("/api/foo"));
