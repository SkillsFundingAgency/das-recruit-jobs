...
[TestFixture]
internal class WhenGettingStaleDraftVacanciesToClose
{
    [Test, MoqAutoData]
        ...
        StaleVacancies response)
    {
        ...
        // act
        await sut.GetDraftVacanciesToCloseAsync(pointInTime, CancellationToken.None);

        // assert
        var request = handler.Requests.Single();
        request.RequestUri.Should().NotBeNull();
        request.RequestUri.Should().Be(new Uri($"http://localhost:8080/vacancies/stale/draft?pointInTime={UrlEncoder.Default.Encode(pointInTime.ToString("s"))}"));
        ...
        StaleVacancies response)
    {
        ...
        // act
        var results = await sut.GetDraftVacanciesToCloseAsync(pointInTime, CancellationToken.None);
