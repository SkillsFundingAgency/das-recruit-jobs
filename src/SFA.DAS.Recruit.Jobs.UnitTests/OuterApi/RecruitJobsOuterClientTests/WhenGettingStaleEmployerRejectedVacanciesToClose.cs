...
[TestFixture]
internal class WhenGettingStaleEmployerRejectedVacanciesToClose
{
    [Test, MoqAutoData]
        ...
        StaleVacancies response)
    {
        ...
        request.RequestUri.Should().Be(new Uri($"http://localhost:8080/vacancies/stale/employer/rejected?pointInTime={UrlEncoder.Default.Encode(pointInTime.ToString("s"))}"));
