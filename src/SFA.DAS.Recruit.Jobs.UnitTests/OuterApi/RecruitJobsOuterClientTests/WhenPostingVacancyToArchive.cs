...
[TestFixture]
internal class WhenPostingVacancyToArchive
{
    [Test, MoqAutoData]
    public async Task Then_The_Request_Is_Correct(Guid vacancyId, long vacancyReference)
    {
        ...
        // act
        await sut.PostVacancyToArchive(vacancyId, vacancyReference, CancellationToken.None);

        // assert
        var request = handler.Requests.Single();
        request.RequestUri.Should().NotBeNull();
        request.RequestUri.Should().Be(new Uri($"http://localhost:8080/vacancies/{vacancyReference}/archive"));
