...
{
    private static readonly DateTime RemigrationCutOff = new(2025, 12, 01,0,0,0); // cause everything to get migrated again
    public async Task<List<VacancyReview>> FetchBatchAsync(int batchSize, int maxAgeInDays)
    {
        ...
        var pipeline = new EmptyPipelineDefinition<VacancyReview>()
            .Match(x => (x.MigrationDate < RemigrationCutOff) && x.MigrationFailed == null && x.MigrationIgnore == null)
            .Limit(batchSize);
