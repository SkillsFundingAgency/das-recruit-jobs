    ...
    : MongoDbCollectionBase(loggerFactory, MongoDbNames.RecruitDb, config, mongoClient)
{
    public async Task<List<ApplicationReview>> FetchBatchAsync(int batchSize)
    {
        ...
        return await RetryPolicy.ExecuteAsync(
            _ => collection
                .Find(x=>x.CreatedDate >= new DateTime(2024,01,01) && x.MigrationDate == null && x.MigrationFailed == null, new FindOptions{MaxTime = TimeSpan.FromMinutes(10), BatchSize = batchSize})
                .Limit(batchSize)
                .ToListAsync(),
