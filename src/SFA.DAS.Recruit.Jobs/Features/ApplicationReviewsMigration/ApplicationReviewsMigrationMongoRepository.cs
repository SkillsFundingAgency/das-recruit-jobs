    ...
    : MongoDbCollectionBase(loggerFactory, MongoDbNames.RecruitDb, config, mongoClient)
{
    public async Task<List<ApplicationReview>> FetchBatchAsync(int batchSize)
    {
        ...
        return await RetryPolicy.ExecuteAsync(
            _ => collection
                .Find(x=> (x.MigrationDate < new DateTime(2025,06,26,0,0,0) || x.MigrationFailed == null) 
                         && x.MigrationFailed == true, new FindOptions{MaxTime = TimeSpan.FromMinutes(10), BatchSize = batchSize})
                .Limit(batchSize)
                .ToListAsync(),
