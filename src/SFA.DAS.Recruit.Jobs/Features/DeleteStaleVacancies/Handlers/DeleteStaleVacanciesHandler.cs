    ...
    IRecruitJobsOuterClient jobsOuterClient) : IDeleteStaleVacanciesHandler
{
    ...
    public async Task RunAsync(CancellationToken cancellationToken)
    {
        ...
        var fetchTasks = await Task.WhenAll(
            ...
            jobsOuterClient.GetEmployerRejectedVacanciesToClose(rejectedStaleByDate, cancellationToken),
            jobsOuterClient.GetQaRejectedVacanciesToClose(rejectedStaleByDate, cancellationToken)
        );
        ...
        AddStaleVacancies(fetchTasks[1], VacancyStatus.Review, referredStaleByDate, totalStaleVacancies);
        AddStaleVacancies(fetchTasks[2], VacancyStatus.Rejected, rejectedStaleByDate, totalStaleVacancies);
        AddStaleVacancies(fetchTasks[3], VacancyStatus.Referred, rejectedStaleByDate, totalStaleVacancies);
